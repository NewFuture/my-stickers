...
export const WelcomePage: React.FC<WelcomePageProps> = props => {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('focus', getFocus) };
  }, [])
  ...
  return (
    <div className={styles.root}>
      ...
      <div className={styles.description}><Text>{loginDescription}</Text></div>
      <Button appearance="primary"
        as='a' disabled={!link} href={link} target="_blank">
        {action}
      </Button>
    </div >
  );
