...
export const WelcomePage: React.FC<WelcomePageProps> = (props) => {
    ...
    useEffect(() => {
        getContext().then((c) => {
            ...
            const link = `https://login.microsoftonline.com/${tenantId}/oauth2/v2.0/authorize?client_id=${process.env.AAD_APP_ID}&response_type=code&redirect_uri=${process.env.REDIRECT_URI}&scope=openid`;
            setLink(link);
        });
    }, []);
    return (
